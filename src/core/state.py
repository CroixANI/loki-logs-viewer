...
def init_state() -> None:
    ...
    if "files" not in st.session_state:
        st.session_state["files"] = {}          # filename -> DataFrame
    if "active_tab" not in st.session_state:
        st.session_state["active_tab"] = None   # currently selected filename
    if "page_sizes" not in st.session_state:
        st.session_state["page_sizes"] = {}     # filename -> int (rows visible)


@st.cache_data(show_spinner=False)
def _cached_parse(file_bytes: bytes) -> pd.DataFrame:
...
def add_file(filename: str, file_bytes: bytes) -> None:
    ...
    if filename in st.session_state["files"]:
        return
    df = _cached_parse(file_bytes)
    st.session_state["files"][filename] = df
